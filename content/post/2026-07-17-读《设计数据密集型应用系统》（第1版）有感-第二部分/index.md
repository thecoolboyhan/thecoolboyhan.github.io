---
title:  读《设计数据密集型应用系统》（第1版）有感-第二部分：分布式数据系统
...
1. 规模的极端是高性能计算，由成千上万个CPU的超级计算机构成了一个庞大的集群，通常用于计算密集型（CPU密集）的任务，天气预报、分子运动计算。

2. 另一个极端是云计算，多个租户数据中心，普通的电脑设备，用网络连接多个中心，弹性（可伸缩）/按需分配资源，按需计费。
...
![1782890800096.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1782890800096_1782890800138.png)
...
![1782891509886.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1782891509886_1782891509905.png)
...
``` java
while (true) { 
	request = getlncomingRequest (); 
	// Ensure that the lease always has at least 10 seconds remaining 
	if (lease.expiryTimeMillis -
	System.currentTime问 ill is() < 10000) { 
		lease = lease . renew();
  }
	if (lease.isValid()) { 
    process(request); 
...
1. 如果本次时间不可能靠，就会出现两个主节点。
2. 如果运行过程中进程暂停，由于垃圾回收器导致的(STW)，会让进程暂停，从而可能会让master在一段时间后才会去检测失效。
...
同样是租约系统，因GC产生的错误：
![1782900357036.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1782900357036_1782900357083.png)
...
![1782900467661.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1782900467661_1782900467678.png)
...
对于拜占庭将军问题，可以利用BTC或Solana的方式来解决。这样的解决方式成本极高，且**必须保证每个节点运行的代码不相同，否则如果可以入侵一个节点，理论上所有节点都可以入侵**。

关于BTC和Solana可以看我的[这篇文章](https://thecoolboyhan.github.io/p/%E5%8C%BA%E5%9D%97%E9%93%BE%E5%85%B1%E8%AF%86%E7%AE%80%E4%BB%8Bbtcethsolana/);
...
![1783332145396.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1783332145396_1783332145404.png)
...
![1783334954895.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1783334954895_1783334954913.png)
...
![1783336698684.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1783336698684_1783336698701.png)
...
![1783337494041.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1783337494041_1783337494063.png)
...
因果一致性可以认为是 ，不会由于网络延迟而显著影响性能，又能对网络故障提供容错的**最强的一致性模型**。
...
1. 每个节点独立产生自己的自增序列号。需预留出表示当前序列号由哪个节点生成的位。
2. 把墙上时钟附加的操作上。墙上时钟可以有效的区分前后。
3. 按步长产生。一次性占用1k个序列号，每次只有达到步长，再重新申请序列号。这样可以使生成的序列号尽量连续，再需要将序列号作为索引的时候，可以更好的进行范围查询。
...
![1784104203214.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1784104203214_1784104203245.png)
...
1. 可靠发送：没有消息丢失，如果消息发送到了一个节点，也必须发送到所有节点。
...
**2PC**是一种在多个节点之间，实现事务原子提交的算法，用来确保所有节点要么全部提交，要么全部中止。

![1784114162760.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1784114162760_1784114162807.png)
...
![1784114949962.png](https://fastly.jsdelivr.net/gh/thecoolboyhan/th_blogs@main/image/2026-07/1784114949962_1784114949994.png)
...
采用XA交易的实现：Transaction注解，消息队列JMS，PostgreSQL, MySQL 、
DB2 、SQL Server和Oracle。ActiveMQ、Horn巳tQ 、MSMQ和IBM MQ ）都支持XA。
...
关于分布式事务，详细的可以看[凤凰架构中的介绍](https://thecoolboyhan.github.io/p/icyfenix-argitektuur/#%E5%88%86%E5%B8%83%E5%BC%8F%E4%BA%8B%E5%8A%A1)。
