...
将交互双方锁涉及的数据转换为某种事先约定好的中立数据流格式来进行传输，将数据流转换回不同语言中对应的数据类型来进行使用。
...
编译器或者解释器会根据语言规范，将调用的方法签名传唤为进程空间中子过程入口位置的指针。
...
4. 分层系统（Layered System）（负载均衡）
   ...
   客户端不需要知道是否直接连接到了最终的服务器，中间服务器可以通过负载均衡和共享缓存的机制提高系统的可扩展性，这样便于缓存、伸缩和安全策略的部署。
...
6. 按需代码（Code-On-Demand）（可选项）
   ...
   客户端无需知道服务端如何运行，服务端会按需把需要的可执行程序发送给客户端执行。
...
> 多个服务同时访问多个数据源的事务处理机制，在分布式场景下的事务处理机制。
...
1. 一致性（C）：数据在任何时刻、任何分布式节点中所看到的都是符合预期的。
...
5. 如果4全部成功，事务最终置为失败，如果4中任意操作出现异常，就重复发送Cancel操作，进行最大努力交付。
...
> 可靠传输机制：如果TCP传输过程中，有一个包出现错误，那所有的流都需要等待这个包重传成功后，才能组装。（为了避免因为少包导致组装出错误的数据）
...
A：分布式缓存每次访问都需要网络通讯，引入分布式加本地的二级缓存机制，只要缓存不失效，从第二次起的所有读取都不再需要网络请求，大大的提高了读取性能。
...
1. 业务逻辑根本就不能避免的缓存穿透，可以约定一定时间内对返回为空的Key值依然进行缓存，使得一段时间内缓存不会被一个key而多次穿透。
   ...
   > 效果并不好，给缓存带来了更大的压力，且对恶意攻击的防御效果并不好。（我可以随机生成key来恶意访问，或查询一些根本不存在的日期的数据，如1800年的数据）
...
3. 签名（一种哈希算法）：对象头中公开的签名算法，通过服务器特定的密钥对前面两个部分加密，而来。
...
| 对称加密   | 加密是指加密和解密是一样的密钥。 设计难度相对较小，执行速度相对较快。 加密明文长度不受限制。 | DES、AES、RC4、IDEA       | 加密           | 要解决如何把密钥安全地传递给解密者。 |
...
3. 客户端确认：
   ...
   客户端利用RSA等算法，生成一个新的随机数。通过三个随机数一起组成对称加密的私钥，后续对称加密都采用此密钥。

4. 服务端确认：

   服务器握手结束通知

> TLS握手结束后，双方只使用随机生成的密钥对称加密来处理请求。
